...
`npm run build` はPages専用のVite設定で `dist` にブラウザー用の静的資産だけを
生成します。ビルド経路自体がWrangler、ローカル用Secret、Workerバンドル、
Worker設定、ソースマップを読み込みません。Pagesはトップレベルに `404.html` がないSPAを自動的に
