...
- 運用制約: Cloudflareへの支払い・決済登録を必要としないFreeプラン構成
...
- 保存期間ポリシー、暗号化ローカルバックアップ、D1 Time Travel、検証付き復元
...
大会発行時に簡易地図を表示し、地図のタップ、中心ピンの移動、端末GPSのいずれかで最初のレース海面を概略指定できるようにする。小数度の直接入力も折りたたみ内に残す。初期提案は経度 `131.5221959`、緯度 `33.2786648` とし、小数点以下7桁まで変更できる。この中心は海面Aと大会作成直後の仮マーク配置の基準として保存し、正確な位置決定とは区別する。
...
3. スタート・ピンとシグナルボートの位置を現在地またはハンディGPSで記録し、決定したスタートラインの中央、風向、風速、目標時間から推奨マーク位置を作成する。
...
- 初回表示、ページ再読込、レース切替では江の島等の固定座標を使わず、選択中レースの保存済み海面中心、次にコースマーク範囲の順で自動表示する。マーク未設定時も大会発行時の概略海面を表示する。

### 7.2 表示レイヤー

| 対象 | 表現 |
|---|---|
| マーク目標地点 | 中抜きのマーク記号、点線のコース。`PIN`、`RC`、`1`、`2`、`3S`、`3P`等の番号・名称を地図上へ常時表示 |
...
| 風 | 観測地点ごとの矢羽、海面平均、振れ幅、鮮度。担当マークボートの最新観測を担当マークへ紐づけ、真風向と `kt / m/s` を併記 |
...
- トラペゾイドは、内側の `1–4` と外側の `2–3` を風軸に平行な2本の風上／風下レグとして配置する。`3S/3P` をスタート–1マーク軸上へ置いて三角形化させない。スタートラインは4ゲートの風下、フィニッシュは3マークからのリーチ側を初期案とする。
...
- スタート・ピンとシグナルボートの両位置が記録済みの場合、その2点を変更せず、測地線上の中点をコース原点として風軸方向へ推奨マーク位置を生成する。片側しか記録されていない場合は実位置と計画位置を混在させず、現在の計画ラインを使うことを明示する。
- 推奨位置の保存前に、使用するラインが「記録した実位置」か「計画位置」か、ライン長、PINからシグナルボートへの真方位を表示する。
...
- 各運営ボートの最新観測をレース別に保持し、担当マークへ表示する。明示的な担当がない場合だけ、2.5km以内の最寄りマークへ「最寄り観測」と明示して参考表示する。
- マーク別風は風向を真方位 `°T`、風速を `kt / m/s` で表示し、5分を超えた観測は古い値として警告色にする。観測艇と経過時間を詳細パネルで確認できるようにする。
...
投下位置はスマートフォンのWeb Geolocationだけでなく、ハンディGPSに表示された座標から手入力できる。海上用GPSで一般的な度＋10進分 `DD°MM.mmmm′` の「分の末尾4桁」を既定とし、10進度 `DD.dddddd°` の末尾4桁および10進度全桁へ明示的に切り替えられるようにする。末尾4桁方式では、現在の投下位置（未投下なら計画位置）の半球、度、分の整数部または10進度の上位桁を固定して転記量を減らす。形式を自動判定せず、選択中の形式と変換後の10進度座標を同時表示する。

ハンディGPS入力では、GPS側の測地系がWGS 84であることを利用者が確認し、入力方式、測地系、機器が表示した精度、任意メモ、実行者、入力時刻を投下・再投下イベントへ保存する。計画位置から1kmを超える入力は緯度経度や形式・桁の取り違えの可能性として警告し、明示確認なしには保存しない。確定済みレースの位置を修正できるのは大会URL発行者だけとし、元イベントを上書きせず「移動／再投下」の追記イベントとして監査ログへ残す。
...
- かんたん表示の先頭に、延期、緊急連絡、確定ロック、計画中、ブロッカー、未確認マークの優先順で「次にやること」を一つ提示し、該当画面へ直接移動できるようにする。
- 大会内の全レースを一つの俯瞰カードに並べ、レース状態、クラス、コース、マーク確認数、必須タスク残数、ブロッカーを表示する。俯瞰カードからレースを切り替えられる。
- 初学者向けに、担当確認、位置共有、目標地点への移動、投下と別艇確認、本部船信号優先の5ステップと、`°T`、`COG`、計画／投下、`kt / m/s` の用語説明を画面内で参照できるようにする。
...
- 日次の1本のCron処理でD1上の削除候補を管理し、Cloudflare無料枠のCron数と操作数を抑える。端末へ保存したバックアップファイルはサーバーから自動削除できないため、大会管理者が保管先ごとに管理する。
...
スマートフォンの通常表示は地図優先を初期値とする。マーク選択時は名称・距離・「マーク操作」だけを1行で表示し、投下、GPS手入力、回収、先頭通過などの操作は利用者が明示的に展開した時だけ地図上へ重ねる。推奨コース長も要約バーへ折りたたみ、マーク選択中は非表示にする。地図と運用ボードの境界には「運用を広げる／地図を広げる」を明記する。
...
- 初期表示は初心者向けの「かんたん」とし、現在フェーズ、コース、マーク、運営ボート、風、先頭通過、緊急連絡を優先する。潮流の詳細、無料枠監視、担当別一覧、時系列は「標準」「詳細」で段階的に開示する。
- 地図には「マークを選ぶ → マーク操作」のように、次に行う操作を短い日本語と番号で常時案内する。専門用語だけのボタンや、意味の分からないアイコン単独表示を避ける。
...
初期実装の状態カードには、選択レースの最新先頭通過時刻・対象マーク・周回・採用状態・観測競合と、未確認の緊急連絡件数を含める。先頭通過は採用値を優先し、未採用なら最新観測候補と明記する。緊急連絡カードから確認・応答画面を直接開く。
...
- 永続データ: D1
- 大容量添付・出力: 初期リリースではサーバーへ本体を保管せず、端末ファイルとハッシュ参照を使用
...
- `race_finalizations`, `post_finalization_revisions`, `post_finalization_revision_drafts`（管理者修正中の未公開差分・基準確定版・対象項目）
...
R2には無料利用量があるが、利用開始にはR2 subscriptionのcheckoutが必要で、無料量超過時の従量課金経路も生じる。支払いを行わないという運用条件を優先し、初期リリースではR2を設定・参照・デプロイしない。
...
- 大会バックアップはサーバーがEd25519署名と監査連番を付け、端末がAES-GCM暗号化して `.srs-backup` としてダウンロードする。平文、パスフレーズ、復号鍵をサーバーへ送らない。
- 大会管理者へ、暗号化ファイルを少なくとも2台の端末または端末と外部メディアへ複製し、パスフレーズを別経路で保管するよう画面で案内する。
- D1 Time TravelはFreeプランで常時有効な過去7日の短期復旧手段として使用する。長期復旧は暗号化ローカルバックアップを正とする。
- 写真・音声のサーバー保管は初期リリースの必須機能から外す。必要な証拠ファイルは端末または主催者管理の保管先に置き、アプリには内容ハッシュ、時刻、説明、参照先だけを記録する。
- 地図はMapLibreと、利用許諾を満たすイベント海域限定のベクトルタイル／PMTilesを使用できる。小規模パックはStatic Assetsまたは端末キャッシュへ置き、課金が必要な地図保管サービスを必須にしない。
- OSM標準タイルへの大量アクセスやオフライン一括取得には依存しない。地図提供元の有料APIを無料枠前提の必須構成に含めない。

#### 19.4.4 使用量監視と縮退

- Workers、Durable Objects、D1の使用率を管理画面に50%、70%、85%、95%の段階で表示する。
- 70%で管理者へ警告し、位置サンプル保存を60秒から120秒へ自動提案する。
- 85%で非必須エクスポートとD1への非重要サンプル保存を停止し、確定イベント用の書込み余裕を確保する。
...
- 初参加者、高齢者、片手操作、濡れた手を想定し、スマートフォンの主要操作は44px以上、補助文は原則10px以上、主要値は14px以上を確保する。
- OSのコントラスト強調設定を尊重し、半透明パネルを不透明化して文字・枠線のコントラストを上げる。
...
43. 大会管理者が署名付き大会データを端末でAES-GCM暗号化して保存でき、別端末のファイルから署名・ハッシュ・監査連番を検証して差分復元できる。D1 Time Travelの無料7日復旧手順も文書化され、R2を設定しなくても本番公開できる。
...
- 選択中コースは常時地図上にコードと名称を表示し、編集権限がある利用者には同じ場所に「コースを選ぶ」を表示する。候補はセレクトボックスだけでなく、コード・名称・用途が見える選択ボタンでも比較できるようにする。
- コース設定は `①コース選択 → ②スタートライン → ③風・ゲート条件 → ④保存・共有` の順に統一する。「選択しただけ」の未保存案と全員へ共有中のコースを明確に分け、保存操作はスクロール位置にかかわらず見えるようにする。
- スナイプはSCIRA Courses 2026に基づく `W2/O2/T2` を別テンプレート群として搭載する。旧版で用いた `W/O/T` は履歴表示用に版情報を保持し、新規大会の初期候補には混在させない。
...
- 許諾済みベクトルタイル／PMTilesをStatic Assetsまたは端末キャッシュから配信し、OSM標準タイルの一括取得に依存しない。
- 初期地図予算は0円とし、Cloudflare無料枠とセルフホスト可能なデータだけを必須構成にする。

### 26.7 保存期間

16.5の初期推奨値を採用する。すなわち、確定・監査記録5年、風・潮流等1年、D1の間引き位置90日、高頻度ローカル航跡7日、通常メッセージ90日、名前・担当1年、招待・復元コード30日、セキュリティログ1年とする。暗号化ローカルバックアップは大会終了後5年間を推奨するが、端末外の保管先は大会管理者が管理する。保存ホールドと大会管理者による版付き変更を認める。
...
Cloudflare Pagesを必須にせず、Workers Static Assets、単一Worker、SQLite-backed Durable Objects、D1の一体構成を採用する。Durable ObjectsもFreeプランで利用でき、Pagesと別Workerの二重デプロイを避けられるためである。静的アセットはWorker実行を経由しない設定とし、R2 subscriptionやWorkers Paidプランを必要としない。
...
- [Garmin GPSMAP 67 Owner's Manual: Position Format Settings](https://www8.garmin.com/manuals/webhelp/GUID-5724F4C0-7557-4C51-BF07-54A567FA4600/EN-US/GUID-89F043D2-983F-4428-88A5-47EF073F4654.html)
- [Garmin GPSMAP Touch Owner's Manual: Position Format and Map Datum](https://www8.garmin.com/manuals/webhelp/gpsmap_touch/EN-GB/GPSMAP_Touch_OM_EN-US.pdf)
