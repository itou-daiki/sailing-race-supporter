...
投下位置はスマートフォンのWeb Geolocationだけでなく、ハンディGPSに表示された座標から手入力できる。初期入力形式は10進度 `DD.dddddd` とし、海上での転記量を減らすため、現在の投下位置（未投下なら計画位置）の度および小数点以下上位2桁を固定し、小数点以下の末尾4桁だけを入力する方式を既定とする。海域または上位桁が異なる場合に備えて全桁入力へ切り替えられるようにする。度分形式は入力値の取り違えを防ぐため、明示的な形式選択を実装するまで自動判定しない。

ハンディGPS入力では、入力方式、機器が表示した精度、任意メモ、実行者、入力時刻を投下・再投下イベントへ保存する。計画位置から1kmを超える入力は緯度経度や桁の取り違えの可能性として警告し、明示確認なしには保存しない。確定済みレースの位置を修正できるのは大会URL発行者だけとし、元イベントを上書きせず「移動／再投下」の追記イベントとして監査ログへ残す。
