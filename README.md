...
`npm run test:all` はブラウザー／ロジックの単体テストに加え、Cloudflareのworkerd上でD1マイグレーション、Durable Objectsの永続化、R2バインディング、Workerエントリーポイントを検証します。
...
本番はCloudflare Workers Static Assetsを使用し、Worker API、SQLite-backed Durable Objects、D1、暗号化バックアップ用R2、日次Cronを同じプロジェクトへ接続します。D1本番データベースは作成・設定・マイグレーション済みです。PagesのGitビルドとWorkers設定は分離済みで、R2の利用開始後にバックアップ用バケットとWorkerを公開します。初回手順は[DEPLOYMENT.md](./DEPLOYMENT.md)を参照してください。
