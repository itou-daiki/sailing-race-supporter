...
O2/I2/W2等の回航順に沿ったコース線、地図上のマーク番号、担当マークボート別の風向・風速（kt / m/s）、大会管理者の地図タップ配置に対応します。再読込時は大会で設定した海面またはレースのマーク範囲へ自動復帰します。運用ボードには、初学者向けの「次にやること」、全レース俯瞰、通信・観測の鮮度警告、5ステップガイドを表示します。
