...
async function createBackup(request: Request, env: AppEnv, eventReference: string): Promise<Response> {
  ...
  const [
    ...
    postFinalizationRevisionDrafts,
    auditEvents,
    retentionRuns,
    retentionTombstones,
    retentionHoldEvents,
    ...
    all(env, 'SELECT schedule.* FROM race_schedule_events schedule JOIN races race ON race.id = schedule.race_id WHERE race.regatta_id = ?', access.eventId),
    ...
    all(env, 'SELECT draft.* FROM post_finalization_revision_drafts draft JOIN races race ON race.id = draft.race_id WHERE race.regatta_id = ?', access.eventId),
